        ...
        for (final WorldConfig wcfg : getLoggedWorlds()) {
            ...
            if (wcfg.isLogging(Logging.KILL))
                createTable(
                        ...
                        state,
                        wcfg.table + "-kills",
                        "(id INT UNSIGNED NOT NULL AUTO_INCREMENT, date DATETIME NOT NULL, killer SMALLINT UNSIGNED, victim SMALLINT UNSIGNED NOT NULL, weapon SMALLINT UNSIGNED NOT NULL, x SMALLINT NOT NULL, y TINYINT UNSIGNED NOT NULL, z SMALLINT NOT NULL, PRIMARY KEY (id))");
        }
        state.close();
        conn.close();
    }
    ...
    boolean update() {
        ...
        if (config.getString("version").compareTo("1.52") < 0) {
            ...
            try {
                ...
                } else getLogger().info("[LogBlock] Column lb-players was alredy modified, skipping it.");
                st.close();
                conn.close();
            } catch (final SQLException ex) {
                Bukkit.getLogger().log(Level.SEVERE, "[LogBlock Updater] Error: ", ex);
