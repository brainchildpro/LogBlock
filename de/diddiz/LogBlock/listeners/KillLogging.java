...
public class KillLogging extends LoggingListener {
    ...
    public void onEntityDamage(final EntityDamageEvent event) {
        if (isLogging(event.getEntity().getWorld(), Logging.KILL) && event instanceof EntityDamageByEntityEvent && event.getEntity() instanceof LivingEntity) {
            ...
            if (this.lastAttackedEntity.containsKey(killer.getEntityId()) && this.lastAttackedEntity.get(killer.getEntityId()) == victim.getEntityId()
                    && System.currentTimeMillis() - this.lastAttackTime.get(killer.getEntityId()) < 5000) return;
