...
public class QueryParams implements Cloneable {
    ...
    private static final Set<Integer> keywords = new HashSet<Integer>(Arrays.asList("player".hashCode(),
            "area".hashCode(), "selection".hashCode(), "sel".hashCode(), "block".hashCode(),
            "type".hashCode(), "sum".hashCode(), "destroyed".hashCode(), "created".hashCode(),
            "chestaccess".hashCode(), "all".hashCode(), "time".hashCode(), "since".hashCode(),
            "before".hashCode(), "limit".hashCode(), "world".hashCode(), "asc".hashCode(),
            "desc".hashCode(), "last".hashCode(), "coords".hashCode(), "silent".hashCode(),
    ...
    public boolean needCount = false, needId = false, needDate = false, needType = false, needData = false,
            needPlayer = false, needCoords = false, needSignText = false, needChestAccess = false,
            needMessage = false;
    ...
    public String getQuery() {
        if (this.bct == BlockChangeType.CHAT) {
            ...
            return select + " " + from + getWhere() + "ORDER BY date " + this.order + ", id " + this.order
                    + " " + getLimit();
        }
        if (this.sum == SummarizationMode.NONE) {
            ...
            return "SELECT type, SUM(created) AS created, SUM(destroyed) AS destroyed FROM ((SELECT type, count(*) AS created, 0 AS destroyed FROM `"
                    + getTable()
                    + "` INNER JOIN `lb-players` USING (playerid) "
                    + getWhere(BlockChangeType.CREATED)
                    + "GROUP BY type) UNION (SELECT replaced AS type, 0 AS created, count(*) AS destroyed FROM `"
                    + getTable()
                    + "` INNER JOIN `lb-players` USING (playerid) "
                    + getWhere(BlockChangeType.DESTROYED)
                    + "GROUP BY replaced)) AS t GROUP BY type ORDER BY SUM(created) + SUM(destroyed) "
                    + this.order + " " + getLimit();
        else return "SELECT playername, SUM(created) AS created, SUM(destroyed) AS destroyed FROM ((SELECT playerid, count(*) AS created, 0 AS destroyed FROM `"
                + getTable()
                + "` "
                + getWhere(BlockChangeType.CREATED)
                ...
                + "` "
                + getWhere(BlockChangeType.DESTROYED)
                + "GROUP BY playerid)) AS t INNER JOIN `lb-players` USING (playerid) GROUP BY playerid ORDER BY SUM(created) + SUM(destroyed) "
                + this.order + " " + getLimit();
    }
    ...
    public String getTitle() {
        ...
        if (this.players.size() > 10)
            title.append((this.excludePlayersMode ? "without" : "from") + " many players ");
        else if (!this.players.isEmpty())
            title.append((this.excludePlayersMode ? "without" : "from") + " player"
                    + (this.players.size() != 1 ? "s" : "") + " "
                    + listing(this.players.toArray(new String[this.players.size()]), ", ", " and ") + " ");
        if (this.match != null && this.match.length() > 0) title.append("matching '" + this.match + "' ");
        if (this.before > 0 && this.since > 0)
            title.append("between " + this.since + " and " + this.before + " minutes ago ");
        else if (this.since > 0)
            title.append("in the last " + this.since + " minutes ");
        else if (this.before > 0) title.append("more than " + this.before * -1 + " minutes ago ");
        if (this.loc != null) {
            if (this.radius > 0)
                title.append("within " + this.radius + " blocks of "
                        + (this.prepareToolQuery ? "clicked block" : "you") + " ");
            else if (this.radius == 0)
                title.append("at " + this.loc.getBlockX() + ":" + this.loc.getBlockY() + ":"
                        + this.loc.getBlockZ() + " ");
        } else if (this.sel != null)
            title.append(this.prepareToolQuery ? "at double chest " : "inside selection ");
        else if (this.prepareToolQuery) if (this.radius > 0)
            title.append("within " + this.radius + " blocks of clicked block ");
        else if (this.radius == 0) title.append("at clicked block ");
        if (this.world != null && !(this.sel != null && this.prepareToolQuery))
            title.append("in " + friendlyWorldname(this.world.getName()) + " ");
        if (this.sum != SummarizationMode.NONE)
            title.append("summed up by " + (this.sum == SummarizationMode.TYPES ? "blocks" : "players")
                    + " ");
    ...
    public String getWhere(BlockChangeType blockChangeType) {
        ...
        if (blockChangeType == BlockChangeType.CHAT) {
            if (this.match != null && this.match.length() > 0) {
                ...
                if (this.match.length() > 3 && !unlike || this.match.length() > 4)
                    where.append("MATCH (message) AGAINST ('" + this.match + "' IN BOOLEAN MODE) AND ");
                else where.append("message " + (unlike ? "NOT " : "") + "LIKE '%"
                        + (unlike ? this.match.substring(1) : this.match) + "%' AND ");
            }
        } else {
            ...
            if (this.loc != null) {
                if (this.radius == 0)
                    where.append("x = '" + this.loc.getBlockX() + "' AND y = '" + this.loc.getBlockY()
                            + "' AND z = '" + this.loc.getBlockZ() + "' AND ");
                else if (this.radius > 0)
                    where.append("x > '" + (this.loc.getBlockX() - this.radius) + "' AND x < '"
                            + (this.loc.getBlockX() + this.radius) + "' AND z > '"
                            + (this.loc.getBlockZ() - this.radius) + "' AND z < '"
                            + (this.loc.getBlockZ() + this.radius) + "' AND ");
            } else if (this.sel != null)
                where.append("x >= '" + this.sel.getMinimumPoint().getBlockX() + "' AND x <= '"
                        + this.sel.getMaximumPoint().getBlockX() + "' AND y >= '"
                        + this.sel.getMinimumPoint().getBlockY() + "' AND y <= '"
                        + this.sel.getMaximumPoint().getBlockY() + "' AND z >= '"
                        + this.sel.getMinimumPoint().getBlockZ() + "' AND z <= '"
                        + this.sel.getMaximumPoint().getBlockZ() + "' AND ");
        }
        ...
        if (this.before > 0)
            where.append("date < date_sub(now(), INTERVAL " + this.before + " MINUTE) AND ");
        if (where.length() > 6)
            where.delete(where.length() - 4, where.length());
    ...
    public void parseArgs(CommandSender sender, List<String> args) throws IllegalArgumentException {
        ...
        for (int i = 0; i < args.size(); i++) {
            ...
            if (param.equals("last")) {
                ...
                for (final String playerName : values)
                    if (playerName.length() > 0) {
                        ...
                        if (playerName.contains("\""))
                            ...
                            this.players.add(matches.size() == 1 ? matches.get(0).getName() : playerName
                ...
                this.loc = new Location(null, Integer.valueOf(vectors[0]), Integer.valueOf(vectors[1]),
                        Integer.valueOf(vectors[2]));
                this.radius = 0;
        ...
        if (!this.prepareToolQuery && this.bct != BlockChangeType.CHAT) {
            ...
            if (!isLogged(this.world))
                throw new IllegalArgumentException("This world ('" + this.world.getName()
                        + "') isn't logged");
        }
