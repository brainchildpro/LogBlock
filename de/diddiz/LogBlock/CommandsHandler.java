...
public class CommandsHandler implements CommandExecutor {
    public abstract class AbstractCommand implements Runnable, Closeable {
        ...
        protected AbstractCommand(CommandSender sender, QueryParams params, boolean async) throws Exception {
            ...
            this.params = params;
            if (async) {
                if (CommandsHandler.this.scheduler.scheduleAsyncDelayedTask(CommandsHandler.this.logblock,
                        this) == -1) throw new Exception("Failed to schedule the command");
            } else run();
        }
            ...
            try {
                ...
                if ((deleted = this.rs.getInt(1)) > 0) {
                    if (!this.params.silent && askClearLogs && this.sender instanceof Player
                            && CommandsHandler.this.questioner != null) {
                        ...
                        if (!CommandsHandler.this.questioner.ask((Player) this.sender,
        ...
        public void run() {
            try {
                ...
                if (!this.params.silent
                        ...
                        && !CommandsHandler.this.questioner.ask((Player) this.sender,
                                "Are you sure you want to continue?", "yes", "no").equals("yes")) {
        ...
        public void run() {
            try {
                ...
                if (!this.params.silent && askClearLogAfterRollback
                        ...
                        && CommandsHandler.this.questioner != null && this.sender instanceof Player) {
                    Thread.sleep(1000);
                    if (CommandsHandler.this.questioner.ask((Player) this.sender,
    ...
    final LogBlockQuestioner questioner;

    CommandsHandler(LogBlock logblock) {
        this.logblock = logblock;
        this.scheduler = logblock.getServer().getScheduler();
        this.questioner = (LogBlockQuestioner) logblock.getServer().getPluginManager()
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args) {
        try {
            if (args.length == 0) {
                ...
                if (command.equals("help")) {
                ...
                } else if (args[0].equalsIgnoreCase("killconnection")) {
                    ...
                    if (this.logblock.hasPermission(sender, "logblock.lookup")) {
                        ...
                        if (command.equals("lookup")) argsList.remove(0);
