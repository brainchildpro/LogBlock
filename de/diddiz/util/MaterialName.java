...
public class MaterialName {
    ...
    static {
        ...
        for (final String entry : cfg.getKeys(false))
            if (isInt(entry)) {
                if (cfg.isString(entry)) materialNames.put(Integer.valueOf(entry), cfg.getString(entry));
                    ...
                    for (final String data : sec.getKeys(false))
                        if (isByte(data)) {
                            if (sec.isString(data)) dataNames.put(Byte.valueOf(data), sec.getString(data));
                            else getLogger().warning("Parsing materials.yml: '" + data + "' is not a string.");
                        } else getLogger().warning("Parsing materials.yml: '" + data + "' is no valid material data");
                } else getLogger().warning("Parsing materials.yml: '" + entry + "' is neither a string nor a section.");
            } else getLogger().warning("Parsing materials.yml: '" + entry + "' is no valid material id");
    }
